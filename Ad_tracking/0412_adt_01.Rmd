...
```{r}
?read.csv
df <- read.csv("train_sample.csv", stringsAsFactors = F, na.string=c("", NA)) # na.string=c( ))
...
## how many of 1(app has been downloaded) from "is_attributed"  
```{r}
sum(is.na(df$is_attributed))  # 0
t_appdown <- table(df$is_attributed)

barplot(t_appdown, col=c("red", "green"),
          names.arg=c("Not downloaded", "downloaded"),
          ylim=c(0, 100000)
          )
