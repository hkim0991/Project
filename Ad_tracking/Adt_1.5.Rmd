...
```{r}
tree_m1 <- tree(is_attributed ~ app + device + os + channel + cl_day + cl_hour + cl_min, data=X_tr)
```
...
tree_m2 <- rpart(is_attributed ~ app + device + os + channel + cl_day + cl_hour + cl_min, data=X_tr, method="class")
plot(tree_m2)
text(tree_m2)
...
tree_m3 <- ctree(is_attributed ~ app + device + os + channel + cl_day + cl_hour + cl_min, data=X_tr)
plot(tree_m3)
```
...
X <- X[tri]
X$y <- factor(ifelse(df$is_attributed[tri] == 0, "0", "1"))
str(X)
```
```{r}
tree_m1 <- tree(y~., data=X)
plot(tree_m1)
text(tree_m1)

cv.tree_m1 <- cv.tree(tree_m1, FUN=prune.misclass)
plot(cv.tree_m1) # no difference

library(e1071)
m1_pred <- predict(tree_m1, X_val, type="class")
confusionMatrix(m1_pred, y_val)
```

```{r}
tree_m2 <- rpart(y~., data=X, method="class")
plot(tree_m2)
text(tree_m2)
...
m2_pred <- predict(tree_m2, X_val, type="class")
confusionMatrix(m2_pred, y_val)

```

```{r}
tree_m3 <- ctree(y~., data=X)
plot(tree_m3)

m3_pred <- predict(tree_m3, X_val)
confusionMatrix(m3_pred, y_val)
```
